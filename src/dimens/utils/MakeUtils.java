...
import dimens.generator.DimenGenerator;
...
public class MakeUtils {
    ...
    private static final String XML_DIMEN_TEMPLETE = "<dimen name=\"DIMEN_%2$d\">%3$.2fdp</dimen>\r\n";
    ...
    public static void makeAll(int designWidth, dimens.constants.DimenTypes type, String buildDir) {
        try {
            ...
            //生成目标目录
            File file = new File(courseFile + buildDir + File.separator + folderName);
            if (!file.exists()) {
                file.mkdirs();
            }

            //生成values文件
            FileOutputStream fos = new FileOutputStream(file.getPath() + File.separator + XML_NAME);
